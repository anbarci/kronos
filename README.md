# Kronos — WordPress Blog & Haber Teması
...
gerektirmez). `Kronos\Core\ContentBot` sınıfı WPCB sabitlerini tanımlar, kendi
autoloader'ını kaydeder ve motoru `after_setup_theme`'de başlatır; tablolar
`after_switch_theme`'de kurulur. Bot admin sayfaları, üst seviye **Kronos** tema
menüsünün altına submenu olarak yerleşir (`kronos_contentbot_parent` filtresiyle
