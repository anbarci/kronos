...
class Watermark {
	...
	public function process( $metadata, $attachment_id ) {
		if ( ! Options::bool( 'kronos_watermark_enable' ) || ! function_exists( 'imagecreatetruecolor' ) ) {
			return $metadata;
		}
		...
		$type    = 'logo' === Options::get( 'kronos_watermark_type' ) ? 'logo' : 'text';
		$logo_id = (int) Options::get( 'kronos_watermark_logo' );
	...
	private function save( $img, string $path, string $mime ): void {
		$q = (int) Options::get( 'kronos_image_quality' );
		$q = ( $q >= 1 && $q <= 100 ) ? $q : 85;
		switch ( $mime ) {
			case 'image/jpeg':
	...
	private function opacity_pct(): int {
		return max( 0, min( 100, (int) Options::get( 'kronos_watermark_opacity' ) ) );
	}

	private function size_pct(): int {
		$s = (int) Options::get( 'kronos_watermark_size' );
		return max( 5, min( 60, $s ?: 18 ) );
	}

	private function position(): string {
		$p     = (string) Options::get( 'kronos_watermark_position' );
	...
	private function font_path(): string {
		$f = (string) apply_filters( 'kronos_watermark_font', '' );
		if ( $f && file_exists( $f ) ) {
			return $f;
		}
		$bundled = KRONOS_DIR . '/assets/fonts/watermark.ttf';
	...
	private function apply_text( $img ): void {
		$text = trim( (string) Options::get( 'kronos_watermark_text' ) );
		if ( '' === $text ) {
