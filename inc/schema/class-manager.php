...
namespace Kronos\Schema;

use Kronos\Core\Options;
use Kronos\Core\Mode;
...
class Manager {
	...
	public function register(): void {
		if ( ! Options::bool( 'kronos_schema_enable' ) || $this->seo_plugin_active() ) {
			return;
		}
	...
	private function article_type(): string {
		$forced = (string) Options::get( 'kronos_schema_type' );
	...
	private function publisher(): array {
		$logo = (int) Options::get( 'kronos_schema_org_logo' );
		if ( ! $logo ) {
			$logo = (int) get_theme_mod( 'custom_logo' );
		}
		$name = (string) Options::get( 'kronos_schema_org_name' );
		$data = [
			'@type' => 'Organization',
