...
class Amp {
	...
	public function register(): void {
		if ( ! Options::bool( 'kronos_amp_enable' ) ) {
			return;
		}
	...
	private function render(): void {
		...
		$css     = file_exists( KRONOS_DIR . '/assets/css/amp.css' )
			? file_get_contents( KRONOS_DIR . '/assets/css/amp.css' )
			: '';
		$thumb   = has_post_thumbnail() ? get_the_post_thumbnail_url( get_the_ID(), 'kronos-hero' ) : '';
...
<body>
	...
	<main>
		<article>
			...
			<?php endif; ?>
			<div class="amp-content<?php echo $truncated ? ' amp-content--cut' : ''; ?>"><?php echo $content; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
			<?php if ( $truncated ) : ?>
				<div class="amp-readmore">
					<p class="amp-readmore__hint"><?php esc_html_e( 'Yazının tamamını okumak için devam edin', 'kronos' ); ?></p>
					<a class="amp-readmore__btn" href="<?php echo esc_url( get_permalink() ); ?>"><?php esc_html_e( 'Yazıyı Oku', 'kronos' ); ?></a>
				</div>
			<?php endif; ?>
		</article>
	</main>
